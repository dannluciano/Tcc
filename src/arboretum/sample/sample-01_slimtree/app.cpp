...
void TApp::Run(){
   ...
   LoadVectorFromFile(QUERYCITYFILE);
	...
	cout << endl;
	cout << "Altura: " << this->SlimTree->GetHeight() << endl;
	cout << "Num Nodes: " << this->SlimTree->GetNodeCount() << endl;
	cout << "Num Objs: " << this->SlimTree->GetNumberOfObjects() << endl;
	cout << "Root: " << this->SlimTree->GetPublicRoot() << endl;
	cout << "Max Occupation: " << this->SlimTree->GetMaxOccupation() << endl;
...
void TApp::PerformQueries(){
   if (SlimTree){
	cout << endl;
      ...
      cout << "\nStarting Statistics for Range Query Genetic with SlimTree.... "<< endl;
      PerformRangeQueryGenetic();
      cout << " Ok\n";
...
//------------------------------------------------------------------------------
void TApp::PerformRangeQueryGenetic(){
   ...
   if (SlimTree){
      ...
      start = clock();
		for (i = 0; i < size; i++){
			result = this->RangeQueryGenetic(queryObjects[i], 0.2);
		}//end for
	  // result = this->RangeQueryGenetic(queryObjects[i], 0.2);
      end = clock();
      cout << "\nTotal Time: " << ((double )end-(double )start) / CLOCKS_PER_SEC << "(s)";
...
}//end TApp::PerformRangeQueryGenetic
...
stResult<TCity> * TApp::RangeQueryGenetic(TCity * queryObject, stDistance range)
{
	stResult<TCity > * result = main_genetic(this->SlimTree, queryObject, range);
	return result;
}